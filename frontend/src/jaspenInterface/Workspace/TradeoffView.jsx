...
// ── Status label ──────────────────────────────────────────────────────────────
const StatusLabel = ({ s }) => {
  const color = s === 'PRIORITIZE' ? ROSE : s === 'HOLD' ? SLATE : MUTED;
  return (
    <span style={{
...
// ── Portfolio row ─────────────────────────────────────────────────────────────
const PortfolioRow = ({ d, alt, onSelect, selected }) => (
  <div
    onClick={() => onSelect(d)}
    style={{
      display: 'grid',
      gridTemplateColumns: '32px 28px 1.7fr repeat(6, 1fr) 56px 88px',
  ...
  >
    ...
    <span style={{ fontFamily:'JetBrains Mono,monospace', fontSize:13, fontWeight:600, color: d.pick ? ROSE : SLATE }}>{d.rank}</span>
    <div style={{
    ...
    <div style={{ minWidth:0 }}>
      <div style={{ fontSize:13, fontWeight: d.pick ? 600 : 500, color:NAVY, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{d.name}</div>
...
const HeroStrip = ({ ideas, objective }) => {
  ...
  const combined    = ideas.reduce((s, d) => s + d.score, 0);
...
const TradeoffView = ({
  ...
  const ideas = useMemo(() => deriveIdeas(scorecardSnapshots), [scorecardSnapshots]);
