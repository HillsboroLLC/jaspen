...
describe('FAQSection Thinking Power copy', () => {
  it('explains plan ranges, shared allowances, and variability', async () => {
    ...
    expect(screen.getByText(/Free 300 supports ~1–2 evaluations/)).toBeInTheDocument();
    expect(screen.getByText(/Team 29,000 shared supports ~165–230 evaluations/)).toBeInTheDocument();
    expect(screen.getByText(/Business 80,000 shared supports ~455–640 evaluations/)).toBeInTheDocument();
  ...
  it('grounds the Free estimate in representative workflow testing', async () => {
    const user = userEvent.setup();
    render(<FAQSection />);

    await user.click(screen.getByRole('button', { name: /free plan just a trial/i }));
    expect(screen.getByText(/representative workflow testing, that supports approximately 1–2 evaluations/i)).toBeInTheDocument();
  });
