    ...
    try:
        ...
        conn.executescript("""
            ...
            CREATE TABLE decision_records (
                ...
                CONSTRAINT fk_decision_records_user_id_users
                    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
            );

            INSERT INTO users (id) VALUES ('u-owner');
...
def _user_id_ondelete(db_path):
    conn = sqlite3.connect(db_path)
    try:
        for row in conn.execute("PRAGMA foreign_key_list(decision_records)"):
            if row[3] == "user_id" and row[2] == "users":
                return row[6]
    finally:
        conn.close()
    return None
...
def test_phase_6_makes_decision_record_attribution_nullable(pre_phase_5_db):
    ...
    assert _user_id_ondelete(pre_phase_5_db) == "SET NULL"


def test_phase_6_preserves_records_when_the_attributed_user_is_deleted(pre_phase_5_db):
    with _migration_ops(pre_phase_5_db):
        _load_phase_5().upgrade()
    with _migration_ops(pre_phase_5_db):
        _load_phase_6().upgrade()
    ...
    try:
        conn.execute("PRAGMA foreign_keys = ON")
        conn.execute("DELETE FROM users WHERE id = 'u-owner'")
        conn.commit()
        rows = conn.execute(
    ...
    assert len(rows) == 2
    assert all(user_id is None for _id, user_id, _decision in rows)
    assert dict((row[0], row[2]) for row in rows)["rec-decided"] == "We will proceed."
...
def test_phase_6_downgrade_succeeds_when_every_record_has_an_author(pre_phase_5_db):
    ...
    assert _user_id_ondelete(pre_phase_5_db) == "CASCADE"
    conn = sqlite3.connect(pre_phase_5_db)
    try:
        assert conn.execute("SELECT COUNT(*) FROM decision_records").fetchone()[0] == 2
    finally:
