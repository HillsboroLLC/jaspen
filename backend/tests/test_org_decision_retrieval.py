...
def test_viewer_can_read_exposure_history_but_cannot_change_record_custody(
    client, app, db, retrieval_cast,
):
    viewer, record = retrieval_cast["viewer"], retrieval_cast["record"]
    record.accepted_exposures = [{"id": "accepted-1", "note": "Known exposure"}]
    db.session.commit()
    headers = _headers(app, viewer)

    listed = client.get(
        "/api/v1/decision-records/from-thread/r-shared/accepted-exposures",
        headers=headers,
    )
    assert listed.status_code == 200
    assert listed.get_json()["accepted_exposures"] == record.accepted_exposures

    refresh = client.post(
        "/api/v1/decision-records/from-thread/r-shared",
        headers=headers,
    )
    accept = client.post(
        "/api/v1/decision-records/from-thread/r-shared/accepted-exposures",
        headers=headers,
        json={"kind": "criterion", "option_name": "option-a", "target_key": "cost"},
    )
    consent = client.post(
        f"/api/v1/decision-records/{record.id}/consent",
        headers=headers,
        json={"library_consent": "named"},
    )
    ...
    assert record.library_consent == "none"
    assert record.accepted_exposures == [{"id": "accepted-1", "note": "Known exposure"}]


def test_outsider_cannot_discover_exposure_history(client, app, retrieval_cast):
    response = client.get(
        "/api/v1/decision-records/from-thread/r-shared/accepted-exposures",
        headers=_headers(app, retrieval_cast["outsider"]),
    )
